...
const AdminHome = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        setOrders(res.data.orders); // assuming backend sends an array directly
      } catch (err) {
  ...
  const handleComplete = async (id) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:3000/api/orders/${id}/serve`, {}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
